...
def parse_greenhouse(company: str, html: str):
...
loc = ""
loc_el = a.find_next(lambda tag: tag.name in ("span", "div") and tag.get("class") and any("location" in c.lower() for c in tag.get("class")))
if loc_el:
...
def parse_generic(company: str, html: str):
soup = BeautifulSoup(html, "html.parser")
jobs: List[Job] = []
...
def harvest_company(company: str, url: str):
try:
html = fetch(url)
...
On Wed, Sep 3, 2025 at 2:55 PM Smith <epiphany.smith@gmail.com> wrote:
from __future__ import annotations
